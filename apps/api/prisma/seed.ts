import { PrismaClient, Role, Gender, SemesterType, DayOfWeek } from '@prisma/client';
...
async function main() {
  ...
  await prisma.systemSetting.createMany({
    data: [
      { key: 'SCHOOL_NAME', value: 'SMA Negeri 1 SIMOGU', description: 'Nama Sekolah Utama' },
  ...
  console.log(`   - Users: 4 (SuperAdmin, Admin, Piket1, Piket2)`);
  console.log(`   - Teachers: ${teachers.length}`);
  console.log(`   - Classes: ${classes.length}`);
  console.log(`   - Periods: ${periods.length}`);
  console.log(`   - Academic Year: 1 (2 Semesters)`);
